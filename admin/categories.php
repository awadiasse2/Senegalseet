...
// 2. TRAITEMENT DU FORMULAIRE : AJOUT D'UNE CATÉGORIE
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_category'])) {
    ...
    if (!empty($name)) {
        try {
            ...
            $check->execute([$name]);
            if ($check->fetch()) {
                $error_msg = "Cette catégorie existe déjà dans le système.";
            } else {
            ...
            $error_msg = "Erreur système lors de la création : " . $e->getMessage();
        }
    } else {
        $error_msg = "Le nom de la catégorie est obligatoire.";
    }
}
...
if (isset($_GET['action']) && $_GET['action'] === 'delete' && isset($_GET['id'])) {
    ...
    try {
        ...
        $error_msg = "Impossible de supprimer cette catégorie. Elle est actuellement liée à des signalements de citoyens.";
    }
}

// Gestion des alertes succès après redirection
if (isset($_GET['success'])) {
    if ($_GET['success'] === 'delete') $success_msg = "Catégorie supprimée avec succès.";
    if ($_GET['success'] === 'add') $success_msg = "Nouvelle catégorie ajoutée avec succès !";
}

// 4. RÉCUPÉRATION DES CATÉGORIES + COMPTEUR DE SIGNALEMENTS
try {
    $query = "SELECT c.id, c.name, COUNT(r.id) as total_reports 
              FROM categories c 
    ...
    $categories = [];
    $error_msg = "Erreur lors du chargement des catégories.";
}
?>
<!DOCTYPE html>
<html lang="fr">
<head>
    ...
    <title>SENEGALSET - Gestion des Catégories</title>
    ...
    <div class="offcanvas offcanvas-start text-white" tabindex="-1" id="sidebarOffcanvas" style="background: var(--sidebar); width: 270px;">
        ...
        <div class="offcanvas-body sidebar-nav p-3" style="overflow-y: auto;">
            ...
            <ul class="nav flex-column mb-3">
                ...
                <li class="nav-item"><a href="categories.php" class="nav-link active"><i class="bi bi-tags"></i> Catégories</a></li>
    ...
    <div class="d-flex w-100">
        ...
        <main class="p-3 p-md-4">
            <header class="d-flex flex-column flex-sm-row justify-content-between align-items-start align-items-sm-center gap-3 mb-4">
                <div>
                    <h4 class="fw-bold m-0 text-dark">Gestion des Catégories</h4>
                    <small class="text-muted">Gérez les types d'incidents et anomalies environnementales</small>
                </div>
                <button type="button" class="btn btn-success btn-sm px-3 rounded-3 shadow-sm" data-bs-toggle="modal" data-bs-target="#addCategoryModal">
            ...
            <div class="card-table-container">
                <div class="h6 fw-bold mb-3" style="font-size:0.85rem;">Typologies actives (<?= count($categories) ?>)</div>
                    ...
                    <table class="table table-hover align-middle mb-0" style="font-size: 0.8rem;">
                        ...
                        <tbody>
                            <?php if(empty($categories)): ?>
                                <tr><td colspan="4" class="text-center py-4 text-muted">Aucune catégorie enregistrée.</td></tr>
                            <?php else: ?>
                                <?php foreach ($categories as $c): ?>
                                <tr>
                                    <td class="text-muted">#<?= $c['id'] ?></td>
                                    <td class="fw-bold text-dark"><i class="bi bi-tag-fill text-muted me-2"></i><?= htmlspecialchars($c['name']) ?></td>
                                    <td class="text-center">
                                        <span class="badge rounded-pill bg-primary bg-opacity-10 text-primary px-3 py-1 fw-bold" style="font-size: 10px;">
                                            <?= (int)$c['total_reports'] ?>
                                        </span>
                                    </td>
                                    <td class="text-end">
                                        <a href="categories.php?action=delete&id=<?= $c['id'] ?>" class="btn btn-sm btn-light text-danger rounded-circle p-1" style="width:28px; height:28px;" onclick="return confirm('Confirmez-vous la suppression de cette catégorie ?');">
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content border-0 shadow-lg" style="border-radius: 16px;">
                <div class="modal-header border-0 bg-light rounded-top-4">
                    <h6 class="modal-title fw-bold m-0"><i class="bi bi-tags text-success me-2"></i>Nouvelle Catégorie</h6>
                    <button type="button" class="btn-close shadow-none" data-bs-dismiss="modal"></button>
                </div>
                <form action="categories.php" method="POST">
                    <div class="modal-body p-4">
                        <div class="mb-3">
                            <label class="form-label text-muted small fw-bold">Intitulé de la catégorie *</label>
                            <input type="text" name="name" class="form-control form-control-sm rounded-3 shadow-none border" placeholder="Ex: Éclairage en panne, Dépôt d'ordures..." required>
                        </div>
                        <small class="text-muted d-block bg-light p-2 rounded" style="font-size:10px;">
                            <i class="bi bi-info-circle-fill text-success"></i> Cette typologie sera directement accessible par les citoyens lors d'un nouveau signalement.
                        </small>
                    </div>
                    <div class="modal-footer border-0 p-3 bg-light rounded-bottom-4">
                        <button type="button" class="btn btn-sm btn-secondary rounded-3" data-bs-dismiss="modal">Annuler</button>
                        <button type="submit" name="add_category" class="btn btn-sm btn-success rounded-3 px-3 shadow-sm">Enregistrer</button>
                    </div>
                </form>
            </div>
        </div>
