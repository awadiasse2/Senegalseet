...
// CORRECTION : Ajout de r.image et sécurisation par user_id
...
<head>
    ...
    <title>Calendrier — SenegalSet</title>
    ...
    <style>
        ...
        /* Miniatures d'images responsives */
        .calendar-event-img {
            width: 75px;
            height: 75px;
            object-fit: cover;
...
<body>
    ...
    <div class="d-flex flex-column flex-lg-row">
            ...
            <div>
                ...
                <div class="menu-section mb-4">
                    ...
                    <ul class="nav flex-column gap-1">
                        ...
                        <li class="nav-item"><a href="calendrier.php" class="nav-link active rounded p-2.5"><i class="bi bi-calendar3 me-2"></i> Calendrier</a></li>
        ...
        <div class="top-bar-mobile justify-content-between align-items-center w-100">
            ...
            <span class="fw-bold text-sm">Calendrier (<?= $total_events_count ?>)</span>
        ...
        <main class="flex-grow-1 p-4 main-content">
            ...
            <header class="d-none d-lg-flex justify-content-between align-items-center mb-4 pb-3 border-bottom">
                <div class="d-flex align-items-center gap-3">
                    <h4 class="m-0 fw-bold text-dark">Calendrier des travaux</h4>
                </div>
                ...
                <?php if (empty($events)): ?>
                    <div class="col-12">
                        <div class="alert alert-info border-0 rounded-4 p-4 text-center shadow-sm">
                            <i class="bi bi-calendar-x d-block fs-3 mb-2"></i> Aucun travail ou intervention de prévu dans votre calendrier pour le moment.
                        </div>
                    </div>
                <?php else: ?>
                    <?php foreach ($events as $event): 
                        if (!empty($event['report_image'])) {
                            $event_img_path = "../uploads/reports/" . $event['report_image'];
                        } else {
                            $event_img_path = "https://images.unsplash.com/photo-1584467541268-b040f83be3fd?auto=format&fit=crop&w=150&q=80";
                        }
                    ?>
                        <div class="col-12 col-md-6">
                            <div class="card p-3 shadow-sm border border-light-subtle rounded-4 bg-white h-100">
                                <div class="d-flex gap-3 align-items-center">
                                    
                                    <!-- Image Miniature de l'Anomalie -->
                                    <img src="<?= htmlspecialchars($event_img_path) ?>" class="calendar-event-img" alt="Illustration intervention">
                                    
                                    <!-- Informations textuelles fluides -->
                                    <div class="flex-grow-1 min-w-0">
                                        <h6 class="fw-bold mb-1 text-dark text-truncate" style="font-size: 14px;">
                                            <?= htmlspecialchars($event['report_title'] ?? 'Intervention technique') ?>
                                        </h6>
                                        <div class="d-flex flex-column gap-1">
                                            <small class="text-muted text-xs text-truncate" style="font-size: 12px;">
                                                <i class="bi bi-clock-history me-1 text-primary"></i> 
                                                <?= date('d/m/Y à H:i', strtotime($event['start_time'])) ?>
                                            </small>
                                            <small class="text-muted text-xs text-truncate" style="font-size: 11px;">
                                                <i class="bi bi-geo-alt me-1 text-danger"></i> 
                                                <?= htmlspecialchars($event['zone_name'] ?? 'Zone non spécifiée') ?>
                                            </small>
                                        </div>
                                    </div>
    ...
    <!-- Scripts Bootstrap indispensables pour le menu mobile -->
